...
<template>
    <div v-if="showEditModal" id='new_dep_modal' class='new-dep-modal' @click.stop>
        ...
        <select class="new-data-input" v-model="depCompany.uuid"> {{ depCompany.uuid }}
            <option :value="depCompany.uuid" disabled selected>{{ depCompany.name }}</option>
            <option v-for="com in companies" :value="com['companyuuid']" :key="com['companyuuid']">{{
                com.companyname }}</option>
        </select>
        
        <h2>New Department name</h2>
        <input class="new-data-input" v-model="editData.departmentName" type="text" placeholder='New Department'>
...
export default {
    ...
    methods: {
        ...
        getEditData(uuid) {
            ...
            apiReq.post("dep/getById", body).then(data => {
                console.log(data[0]);
                if (data.length > 0) {
                    for (let i = 0; i < data.length; i++) {
                        this.depCompany.name = data[i].companyname;
                        this.depCompany.uuid = data[i].companyuuid;
                        this.editData.companyUUID = data[i].companyuuid;
                        this.editData.departmentUUID = data[i].departmentuuid;
                        this.editData.departmentName = data[i].departmentname;
                    }
                }
            }).catch(err => {
                console.log(err);
                alert("no dep data found");
            })
        },
        editSelected() {
            this.editData.companyUUID = this.depCompany.uuid;
            const body = {
                department: this.editData
            }
            apiReq.put("dep/edit", body).then(data => {
                console.log(data);
    ...
    mounted() {
        this.getCompanyData();
        console.log("show modal is ", this.showModal);
